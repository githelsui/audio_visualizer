...
function drawVisualizer() {
  ...
  dataArray.forEach((item, index) => {
    ...
    var data_label = (item / 255 * height / 2).toPrecision(9)
    const y = (item / 255 * height / 2) * 2.3
    const x = barWidth * index

    canvasContext.fillStyle = `hsl(0, 100%, 99%)`
    // canvasContext.BiquadFilterNode
    canvasContext.strokeStyle = 'black'
    canvasContext.strokeRect(x, height - y, barWidth, y)
    canvasContext.fillRect(x, height - y, barWidth, y)

    //data text labels
    canvasContext.font =  "18px monospace";
    canvasContext.lineWidth = 1;
    canvasContext.fillStyle = '#f7f1df'
    canvasContext.fillText(data_label, x, height - y);
  })
}
